...
exports.autoConfirmStaleEscrows = autoConfirmStaleEscrows;
...
async function autoConfirmStaleEscrows() {
    const due = await prisma_1.prisma.transaction.findMany({
        where: {
            status: "ESCROWED",
    ...
    for (const txn of due) {
        ...
        // same job.
        const confirmedRows = await prisma_1.prisma.$executeRaw `
      UPDATE "Transaction"
    ...
    `;
        if (confirmedRows > 0) {
            confirmedCount += 1;
            const reason = !txn.buyerCheckInAt && txn.sellerCheckInAt
...
//# sourceMappingURL=autoConfirmStaleEscrows.js.map