...
async function matchAlerts() {
    ...
    const newListings = await prisma_1.prisma.listing.findMany({
        where: {
    ...
    for (const listing of newListings) {
        const origin = client_1.Prisma.sql `ST_SetSRID(ST_MakePoint(${listing.theaterLng}, ${listing.theaterLat}), 4326)::geography`;
      ...
      WHERE ta."isActive" = true
        ...
        AND similarity(ta."titleQuery", ${listing.movieName}) > ${searchMatch_1.TITLE_SIMILARITY_THRESHOLD}
    ...
    `;
        for (const match of matches) {
            try {
                await sms_1.smsProvider.send(match.phone, `GrabMySeats: a listing matching "${listing.movieName}" just showed up near you. Open the app before it's gone.`);
                ...
                await prisma_1.prisma.alertNotification.create({
                    data: { alertId: match.alertId, listingId: listing.id },
                });
                notifiedCount += 1;
            }
            catch (err) {
                console.error(`[jobs] matchAlerts failed for alert ${match.alertId} / listing ${listing.id}`, err);
            }
        }
    }
    return notifiedCount;
}
//# sourceMappingURL=matchAlerts.js.map