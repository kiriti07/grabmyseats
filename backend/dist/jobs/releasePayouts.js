...
exports.releasePayouts = releasePayouts;
...
async function releasePayouts() {
    ...
    for (const { id } of due) {
        let transferId;
        try {
            ({ transferId } = await payments_1.payoutProvider.releaseTransfer(id));
        }
        catch (err) {
            console.error(`[jobs] releaseTransfer failed for transaction ${id}`, err);
            continue;
        }
        const releasedRows = await prisma_1.prisma.$executeRaw `
      UPDATE "Transaction"
...
//# sourceMappingURL=releasePayouts.js.map