...
exports.cropPosterRegion = cropPosterRegion;
...
async function cropPosterRegion(buffer) {
    ...
    const mask = Buffer.from(`<svg width="${width}" height="${height}"><rect x="0" y="0" width="${posterBoxWidth}" height="${posterBoxHeight}" fill="white" /></svg>`);
    return image.composite([{ input: mask, top: 0, left: 0 }]).toBuffer();
}
//# sourceMappingURL=cropPosterRegion.js.map