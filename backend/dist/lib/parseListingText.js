...
exports.parseListingText = parseListingText;
...
const TITLE_BLOCKLIST_RE = /your\s*ticket|m-?ticket|watch\s*trailer|tap\s*for\s*support|details\s*&\s*more\s*actions|enable\s*live\s*updates|cancellation\s*not\s*available|find\s*venue|add\s*to\s*google\s*wallet|you'?ve\s*won|tap\s*to\s*open|tap\s*to\s*refresh|something\s*went\s*wrong|screen\s*\d+/i;
...
function extractBookingIdFromLabel(text) {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (inline.length >= 4 && inline.length <= 20)
            return inline.toUpperCase();
        const next = lines[i + 1]?.replace(/[^A-Za-z0-9]/g, "");
        if (next && next.length >= 4 && next.length <= 20)
...
function extractBookingIdFallback(text, claimed) {
    const claimedText = claimed.filter((c) => !!c);
    for (const line of linesOf(text)) {
        const candidate = line.toUpperCase();
        if (!BOOKING_ID_CODE_RE.test(candidate))
            continue;
        if (claimedText.includes(candidate))
            continue;
        return candidate;
    }
    return null;
}
function extractBookingId(text, claimed = []) {
    return extractBookingIdFromLabel(text) ?? extractBookingIdFallback(text, claimed);
}
// "N Ticket(s)"
function extractSeatCountFromLabel(text) {
...
function extractSeatCountFromList(text) {
...
function extractTotalAmountPaid(text) {
...
function extractShowtime(text) {
    const match = text.match(/(?:[A-Za-z]{3,9},?\s*)?(\d{1,2})\s+([A-Za-z]{3,9})\D{0,6}?(\d{1,2}):(\d{2})\s*([AaPp][Mm])/);
    if (!match)
        return null;
...
function extractTheaterName(text) {
    for (const line of linesOf(text)) {
...
function extractMovieName(lines, claimed) {
    const claimedText = claimed.filter((c) => !!c);
    const candidates = lines.filter((line) => {
        if (line.confidence < MIN_LINE_CONFIDENCE)
            return false;
        if (line.text.length < 2 || line.text.length > 60)
            return false;
        if (!/[A-Za-z]{2,}/.test(line.text))
            return false;
        if (MOVIE_NAME_EXCLUDE_RE.test(line.text))
            return false;
        if (TITLE_BLOCKLIST_RE.test(line.text))
            return false;
        if (STATUS_BAR_RE.test(line.text))
            return false;
        if (CODE_LIKE_RE.test(line.text))
            return false;
        if (FILE_CHROME_RE.test(line.text))
            return false;
        if (claimedText.some((c) => line.text.includes(c)))
            return false;
        return true;
    });
...
function parseListingText(text, lines) {
...
//# sourceMappingURL=parseListingText.js.map