...
exports.RazorpayPayoutProvider = void 0;
...
class RazorpayPayoutProvider {
    async createLinkedAccount(details) {
        const account = await razorpay_1.razorpay.accounts.create({
            email: details.email,
        ...
        try {
            const product = await razorpay_1.razorpay.products.requestProductConfiguration(account.id, {
                product_name: "route",
                tnc_accepted: true,
            });
            await razorpay_1.razorpay.products.edit(account.id, product.id, {
                settlements: {
            ...
            console.error(`[payments] account ${account.id} created but Route product configuration failed`, productErr);
        }
        return { accountId: account.id };
    }
    async createEscrowOrder(amount, currency) {
    ...
    async releaseTransfer(transactionId) {
        const transaction = await prisma_1.prisma.transaction.findUniqueOrThrow({
            where: { id: transactionId },
            include: { listing: { include: { seller: true } } },
        });
        const sellerAccountId = transaction.listing.seller.razorpayAccountId;
        if (!sellerAccountId) {
            throw new Error(`Cannot release transfer for transaction ${transactionId}: seller has no linked payout account`);
        }
        ...
        const transfer = await razorpay_1.razorpay.transfers.create({
    ...
    async refund(transactionId) {
        ...
        if (!transaction.razorpayOrderId) {
            throw new Error(`Cannot refund transaction ${transactionId}: no razorpayOrderId on record`);
        }
        const { items } = await razorpay_1.razorpay.orders.fetchPayments(transaction.razorpayOrderId);
        const captured = items.find((payment) => payment.status === "captured");
        if (!captured) {
            throw new Error(`Cannot refund transaction ${transactionId}: no captured payment found for order ${transaction.razorpayOrderId}`);
        }
        ...
        const refund = await razorpay_1.razorpay.payments.refund(captured.id, {
            amount: Math.round(transaction.amountPaid * 100),
        });
        return { refundId: refund.id };
    }
}
exports.RazorpayPayoutProvider = RazorpayPayoutProvider;
//# sourceMappingURL=RazorpayPayoutProvider.js.map