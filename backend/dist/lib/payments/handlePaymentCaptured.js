...
async function handlePaymentCaptured(transaction, orderId) {
    ...
    const escrowed = await prisma_1.prisma.$queryRaw `
    UPDATE "Transaction"
  ...
  `;
    ...
    const latest = await prisma_1.prisma.transaction.findUnique({ where: { id: transaction.id } });
    ...
    console.error(`[payments] flagged transaction ${transaction.id} (order ${orderId}) for manual review`);
    return { message: "Payment captured but reservation expired - flagged for manual review" };
}
//# sourceMappingURL=handlePaymentCaptured.js.map