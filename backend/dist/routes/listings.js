...
const ALL_DELIVERY_METHODS = ["IN_PERSON", "EMAIL_FORWARD"];
...
function parseDeliveryMethods(value) {
    ...
    const allValid = methods.every((m) => ALL_DELIVERY_METHODS.includes(m));
    return allValid ? methods : null;
}
...
exports.listingsRouter.post("/", auth_1.requireAuth, upload_1.uploadScreenshot, async (req, res) => {
    ...
    if (!theaterName)
        errors.push("theaterName is required");
    if (!bookingId)
    ...
    if (totalAmountPaid !== null &&
        ...
        errors.push(`Your listed price (₹${listedTotal.toFixed(2)} total) is higher than what you paid for this ticket (₹${totalAmountPaid.toFixed(2)}) — sellers can only recover up to what they originally paid.`);
    }
    if (errors.length > 0) {
    ...
    if (!coordsSupplied) {
        ...
        const query = [(0, cleanTheaterName_1.cleanTheaterName)(theaterName), city, "India"].filter(Boolean).join(", ");
        const geocoded = await (0, geocode_1.geocodeAddress)(query);
        if (!geocoded) {
            const body = {
                success: false,
    ...
    const { url: screenshotUrl } = await storage_1.storageProvider.upload(req.file.buffer, filename);
    ...
    let listing;
    try {
        listing = await prisma_1.prisma.listing.create({
            data: {
    ...
    const body = {
        success: true,
        data: { listing: (0, serialize_1.toSharedListing)(listing) },
    };
...
exports.listingsRouter.post("/ocr", auth_1.requireAuth, upload_1.uploadScreenshot, async (req, res) => {
    if (!req.file) {
        const body = { success: false, error: "screenshot is required" };
        res.status(400).json(body);
        return;
    }
    const category = parseCategory(req.body?.category);
    ...
    const [extraction, qrData] = await Promise.all([
        (0, ocr_1.extractTextFromImage)(req.file.buffer, { skipPosterCrop: !isMovie }).catch(() => ({
            text: "",
            lines: [],
        })),
        (0, qr_1.decodeQrCode)(req.file.buffer).catch(() => null),
    ]);
    const fields = isMovie ? (0, parseListingText_1.parseListingText)(extraction.text, extraction.lines) : EMPTY_OCR_FIELDS;
    const body = {
        success: true,
        data: { fields, qrData },
    };
...
exports.listingsRouter.post("/geocode", auth_1.requireAuth, async (req, res) => {
    const theaterName = requireNonEmptyString(req.body?.theaterName);
    const city = requireNonEmptyString(req.body?.city);
    if (!theaterName) {
        const body = { success: false, error: "theaterName is required" };
        res.status(400).json(body);
        return;
    }
    const cleanedName = (0, cleanTheaterName_1.cleanTheaterName)(theaterName);
...
const ACTIVE_STATUSES = client_1.Prisma.sql `('ACTIVE', 'PARTIALLY_SOLD')`;
...
exports.listingsRouter.get("/search", async (req, res) => {
    const errors = [];
    ...
    if (typeof req.query.showtimeFrom === "string" && req.query.showtimeFrom.trim() !== "") {
        ...
        if (Number.isNaN(d.getTime()))
            errors.push("showtimeFrom must be a valid date");
        else
            showtimeFrom = d;
    }
    let showtimeTo;
    if (typeof req.query.showtimeTo === "string" && req.query.showtimeTo.trim() !== "") {
        ...
        if (Number.isNaN(d.getTime()))
            errors.push("showtimeTo must be a valid date");
        else
            showtimeTo = d;
    }
    ...
    const origin = client_1.Prisma.sql `ST_SetSRID(ST_MakePoint(${lng}, ${lat}), 4326)::geography`;
    const conditions = [
        client_1.Prisma.sql `status IN ${ACTIVE_STATUSES}`,
        client_1.Prisma.sql `"availableSeats" >= ${minSeats}`,
        client_1.Prisma.sql `"theaterLocation" IS NOT NULL`,
        client_1.Prisma.sql `ST_DWithin("theaterLocation", ${origin}, ${radiusMeters})`,
        ...
        client_1.Prisma.sql `NOT EXISTS (
    ...
    if (movieName) {
        conditions.push(client_1.Prisma.sql `(similarity("movieName", ${movieName}) > ${searchMatch_1.TITLE_SIMILARITY_THRESHOLD} OR similarity("theaterName", ${movieName}) > ${searchMatch_1.TITLE_SIMILARITY_THRESHOLD})`);
    }
    if (category)
        conditions.push(client_1.Prisma.sql `category = ${category}::"Category"`);
    if (showtimeFrom)
        conditions.push(client_1.Prisma.sql `showtime >= ${showtimeFrom}`);
    if (showtimeTo)
        conditions.push(client_1.Prisma.sql `showtime <= ${showtimeTo}`);
    ...
    const orderClause = isWideRadiusCategory
        ? client_1.Prisma.sql `ORDER BY GREATEST(similarity("movieName", ${movieName ?? ""}), similarity("theaterName", ${movieName ?? ""})) DESC, "distanceKm" ASC`
        : client_1.Prisma.sql `ORDER BY "distanceKm" ASC`;
...
exports.listingsRouter.get("/mine", auth_1.requireAuth, async (req, res) => {
    ...
    const body = {
        success: true,
        data: { listings: listings.map(serialize_1.toMyListing) },
    };
    res.json(body);
});
...
exports.listingsRouter.get("/:id", async (req, res) => {
    ...
    if (!listing || listing.seller.suspendedAt) {
        const body = { success: false, error: "Listing not found" };
        res.status(404).json(body);
        return;
    }
    const distanceKm = lat !== null && lng !== null
        ? (0, haversine_1.haversineDistanceMeters)(lat, lng, listing.theaterLat, listing.theaterLng) / 1000
        : null;
    const sellerRatingSummary = await (0, ratingSummary_1.getRatingSummary)(listing.sellerId);
    const body = {
        success: true,
        data: { listing: (0, serialize_1.toListingDetail)(listing, distanceKm, sellerRatingSummary) },
    };
    res.json(body);
});
...
exports.listingsRouter.post("/:id/reserve", auth_1.requireAuth, async (req, res) => {
    const listingId = req.params.id;
    const seats = requireFiniteNumber(req.body?.seats);
    const requestedDeliveryMethod = typeof req.body?.deliveryMethod === "string" &&
        ALL_DELIVERY_METHODS.includes(req.body.deliveryMethod)
    ...
    if (config_1.PAYMENT_MODE === "escrow" && !requestedDeliveryMethod) {
    ...
    try {
        const { transaction, sellerId } = await prisma_1.prisma.$transaction(async (tx) => {
            let pricePerSeat;
            let sellerId;
        ...
        `;
                ...
                const listing = await tx.listing.findFirst({
                    where: {
                        id: listingId,
                        status: { in: shared_1.LIVE_LISTING_STATUSES },
            ...
            if (sellerId === req.user.id)
                throw new CannotReserveOwnListingError();
            ...
            // this mode.
            const deliveryMethod = config_1.PAYMENT_MODE === "escrow" ? requestedDeliveryMethod : "IN_PERSON";
            if (config_1.PAYMENT_MODE === "escrow" && !availableDeliveryMethods.includes(deliveryMethod)) {
            ...
            const transaction = await tx.transaction.create({
                data: {
                    ...
                    reservationExpiresAt: config_1.PAYMENT_MODE === "escrow"
        ...
        let contact = null;
        if (config_1.PAYMENT_MODE === "contact_only") {
            const seller = await prisma_1.prisma.user.findUniqueOrThrow({
                where: { id: sellerId },
            ...
            contact = {
                ...
                ratingSummary: await (0, ratingSummary_1.getRatingSummary)(sellerId),
            };
        }
        const body = {
            success: true,
            data: { transaction: (0, serialize_1.toSharedTransaction)(transaction), contact, paymentMode: config_1.PAYMENT_MODE },
        };
        res.status(201).json(body);
    }
    catch (err) {
...
exports.listingsRouter.post("/:id/mark-sold", auth_1.requireAuth, async (req, res) => {
    const listingId = req.params.id;
    const seats = requireFiniteNumber(req.body?.seats);
    if (seats === null || seats <= 0 || !Number.isInteger(seats)) {
        const body = {
    ...
    try {
        const listing = await prisma_1.prisma.$transaction(async (tx) => {
            ...
            const consumed = await (0, listingSeats_1.consumeListingSeats)(tx, listingId, seats);
            if (!consumed)
                throw new InsufficientSeatsError();
...
exports.listingsRouter.post("/:id/deactivate", auth_1.requireAuth, async (req, res) => {
    const listingId = req.params.id;
    try {
        const listing = await prisma_1.prisma.$transaction(async (tx) => {
            const existing = await tx.listing.findUnique({
            ...
            if (existing.sellerId !== req.user.id)
                throw new NotListingOwnerError();
            if (!shared_1.LIVE_LISTING_STATUSES.includes(existing.status))
            ...
            const updated = await tx.listing.updateMany({
                where: { id: listingId, status: { in: shared_1.LIVE_LISTING_STATUSES } },
...
exports.listingsRouter.patch("/:id", auth_1.requireAuth, async (req, res) => {
    const listingId = req.params.id;
    ...
    try {
        const listing = await prisma_1.prisma.$transaction(async (tx) => {
            ...
            if (existing.totalAmountPaid !== null &&
                ...
                throw new PriceIntegrityUpdateError(`Your listed price (₹${listedTotal.toFixed(2)} total) would be higher than what you paid for this ticket (₹${existing.totalAmountPaid.toFixed(2)}) — sellers can only recover up to what they originally paid.`);
            }
            const data = {};
